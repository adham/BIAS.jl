#=
demo_RCRF_MultinomialDirichlet_sparse.jl
...
xx = Array(Vector{Vector{Sent}}, TT)
for tt = 1:TT
    xx[tt] = Array(Vector{Sent}, n_groups[tt])

    for jj = 1:n_groups[tt]
        xx[tt][jj] = Array(Sent, n_group_j[tt][jj])
        for ii = 1:n_group_j[tt][jj]
            kk = true_zz[tt][jj][ii]
            sentence = sample(true_topics[kk, :][:], n_tokens)
            xx[tt][jj][ii] = BIAS.sparsify_sentence(sentence)
        end
    end
end
...
n_internals = 10
store_every = 1000
filename    = "demo_RCRF_MultinomialDirichlet_sparse_"
